...
class db {
	...
	public function __construct($conf){
		...
		}catch(Exception $e){throw new \graceException('数据库连接失败','请检查数据库相关配置');}
	}
	...
	public function add($data = null){
		...
		if(!is_array($data)){throw new \graceException('插入数据错误','插入数据应为一个一维数组');}
	...
	public function delete(){
		$startTime = microtime(true);
		if(empty($this->where)){throw new \graceException('请使用模型对象的 where() 函数设置删除条件');}
		$where              = $this->getWhere();
	...
	public function update($data=null){
		...
		if(empty($data) || !is_array($data)){throw new \graceException('update($data) 函数的参数应该为一个一维数组');}
		if(empty($this->where)){throw new graceException('请使用模型对象的 where() 方法设置更新条件');}
		$where = $this->getWhere();
